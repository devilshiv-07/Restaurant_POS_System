  ...
  return (
    <div className='container mx-auto px-6 md:px-4'>
      <div className='flex justify-between items-center'>
        <div>
          <h2 className='text-[#f5f5f5] font-semibold text-lg'>Overall Performance</h2>
          <p className="text-sm text-[#ababab]">
            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Amet, doloribus?
          </p>
        </div>
        <button className='flex items-center text-sm gap-1 px-6 py-2.5 rounded-md text-[#f5f5f5] bg-[#1a1a1a]'>
          Last 1 Month
          <svg
            className="w-3 h-3"
            viewBox="0 0 24 24"
        ...
        {metricsData.map((metric, index) => {
          return (
            ...
            >
              ...
              <p className="mt-1 font-semibold text-xl text-[#f5f5f5]">
                {metric.value}
              </p>
            </div>
          );
...

            {
                itemsData.map((item, index) => {
                    return (
                        ...
                        <p className="mt-1 font-semibold text-xl text-[#f5f5f5]">{item.value}</p>
                      </div>
                    )
                })
            }
