...
const BottomNav = () => {
  ...
  return (
    <div className='fixed bottom-0 left-0 right-0 bg-[#262626] p-2 h-12 flex justify-around'>
      ...
      {/* Order Button */}
      <button onClick={openModal} className='absolute bottom-6 bg-[#F6B100] text-[#f5f5f5] rounded-full p-2'>
        <BiSolidDish size={20}/>
      </button>
      ...
      {/* name entry */}
      <div>
          <label className='ml-1 block text-[#ababab] mb-2 text-sm font-medium mt-2'>Customer Name</label>
          <div className='flex items-center rounded-full py-2 px-4 bg-[#1f1f1f]'>
            <input type="text" name="" placeholder='Enter cumtomer name' id="" className='bg-transparent flex-1 text-white text-sm focus:outline-none'/>
          </div>
        </div>

        {/* phone detail */}
        <div>
          <label className='ml-1 block text-[#ababab] mb-2 text-sm font-medium mt-4'>Customer Phone</label>
          <div className='flex items-center rounded-full py-2 px-4 bg-[#1f1f1f]'>
            <input type="number" name="" placeholder='+91-9999999999' id="" className='bg-transparent flex-1 text-white text-sm focus:outline-none'/>
          </div>
        </div>

        {/* No of people */}
        <div>
          <label className='ml-1 block text-[#ababab] mb-2 text-sm font-medium mt-4'>Number of guests</label>
          <div className='flex items-center justify-between rounded-full px-4 py-1 bg-[#1f1f1f] text-sm'>
            <button onClick={decrement} className='text-yellow-500 text-lg'>&minus;</button>
            <span className='text-white'>{guestCount} Person</span>
            <button onClick={increment} className='text-yellow-500 text-lg'>&#43;</button>
          </div>
        </div>

        {/* Submit button */}
        <button onClick={() => navigate("/tables")} className='w-full bg-[#f6b100] text-[#f5f5f5] rounded-full p-1 mt-6 hover:bg-yellow-600'>
          Create Order
        </button>
      </Modal> 
    </div>
